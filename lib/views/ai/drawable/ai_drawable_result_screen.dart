...
class _AiDrawableResultScreenState extends AppState<AiDrawableResultScreen> {
  ...
  generate() async {
    ...
    uploadImageController.uploadCompressedImage(compressedImage).then((value) {
      if (value) {
        progressBarController.uploadComplete();
        api
            .draw(
          text: drawableController.textEditingController.text,
          ...
          if (value != null) {
            ...
            setState(() {
              resultFilePaths = value.filePath;
              resultFilePath = resultFilePaths.first;
            });
          } else {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Row(
            children: resultFilePaths.transfer((e, index) => ClipRRect(
                  child: Image(
          ...
          Row(
            children: [
              Expanded(
                  child: buildButton(context, icon: Images.ic_share_discovery, text: S.of(context).tabDiscovery, onTap: () {
                AppDelegate.instance.getManager<UserManager>().doOnLogin(context, logPreLoginAction: 'share_discovery_from_ai_draw', callback: () {
                  ...
                  ShareDiscoveryScreen.push(
                    ...
                    if (value ?? false) {
                      Events.metaverseCompleteShare(source: photoType == 'recently' ? 'recently' : 'ai_draw', platform: 'discovery', type: 'image');
              ...
              Expanded(
                  child: buildButton(context, icon: Images.ic_share, text: S.of(context).share, onTap: () async {
                await showLoading();
                if (TextUtil.isEmpty(resultFilePath)) {
                  return;
                }
                var uint8list = await ImageUtils.printAiDrawData(localImage, File(resultFilePath!), '@${userManager.user?.getShownName() ?? 'Pandora User'}');
                ...
                ShareScreen.startShare(context,
                  ...
                  Events.metaverseCompleteShare(source: photoType == 'recently' ? 'recently' : 'ai_draw', platform: platform, type: 'image');
                });
          ...
          buildButton(context, icon: Images.ic_download, text: S.of(context).download, onTap: () async {
            await showLoading();
            var uint8list = await ImageUtils.printAiDrawData(localImage, File(resultFilePath!), '@${userManager.user?.getShownName() ?? 'Pandora User'}');
  ...
  Widget buildButton(BuildContext context, {required String icon, required String text, required GestureTapCallback onTap}) {
    return Row(
