...
class CubicToCommand extends PathCommand {
  ...
  const CubicToCommand(this.x1, this.y1, this.x2, this.y2, this.x3, this.y3) : super._(PathCommandType.cubic);

  /// Creates a cubic command from the current point to [end] using [control1]
  /// and [control2] as control points.
  CubicToCommand.fromOffsets(Offset control1, Offset control2, Offset end) : this(control1.dx, control1.dy, control2.dx, control2.dy, end.dx, end.dy);
  ...
  double computeLength(Offset start) {
    ...
    double _compute(
      ...
      if (_offsetDistance(cp1, Offset.lerp(p1, p2, 1 / 3)!) > tolerance || _offsetDistance(cp2, Offset.lerp(p1, p2, 2 / 3)!) > tolerance) {
  ...
  @override
  bool operator ==(Object other) {
    return other is CubicToCommand && other.x1 == x1 && other.y1 == y1 && other.x2 == x2 && other.y2 == y2 && other.x3 == x3 && other.y3 == y3;
  }
...
class PathBuilder {
  ...
  PathBuilder addOval(Rect oval) {
    ...
    // Top right arc.
    cubicTo(c.dx + m.dx, c.dy - r.dy, c.dx + r.dx, c.dy - m.dy, c.dx + r.dx, c.dy);

    // Bottom right arc.
    cubicTo(c.dx + r.dx, c.dy + m.dy, c.dx + m.dx, c.dy + r.dy, c.dx, c.dy + r.dy);
  ...
  PathBuilder addRRect(Rect rect, double rx, double ry) {
    ...
    // Bottom left arc.
    cubicTo(rect.left + rx - magicRadius.dx, rect.top + rect.height, rect.left, rect.top + rect.height - ry + magicRadius.dy, rect.left, rect.top + rect.height - ry);
