...
class ChoosePhotoScreen extends StatefulWidget {
  ...
  ChoosePhotoScreen({Key? key, required this.list, required this.pos})
      : super(key: key);
...
class _ChoosePhotoScreenState extends State<ChoosePhotoScreen> {
  ...
  final ChoosePhotoScreenController controller =
      Get.put(ChoosePhotoScreenController());
      ...
      if(itemPos != ((widget.pos == widget.list.length - 1)? widget.pos : itemPositionsListener.itemPositions.value.first.index)){
        ...
        try {
          ...
          // if(scrollController1.hasClients){
            scrollController1.scrollTo(
                index: (widget.pos == widget.list.length - 1)? widget.pos : (itemPositionsListener.itemPositions.value.first.index > 0)? itemPositionsListener.itemPositions.value.first.index - 1 : 0,
                duration: Duration(milliseconds: 100),
                curve: Curves.easeInOutCubic);
          // }
        } catch (error) {
                  ...
                  child: Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          TitleTextWidget(
                              StringConstant.exit_msg,
                              ColorConstant.BtnTextColor,
                              FontWeight.w600,
                              16.sp),
                          SizedBox(height: 0.3.h,),
                          TitleTextWidget(
                              StringConstant.exit_msg1,
          ...
          child: SafeArea(
            child: Column(
              children: [
                ...
                Obx(() => Expanded(
                  child: (controller.isPhotoDone.value)? SingleChildScrollView(
                    child: Column(
                      children: [
                        Padding(
                          padding: EdgeInsets.symmetric(vertical: 2.h),
                          child: (controller.isVideo.value)? AspectRatio(
                        ...
                        Visibility(visible: false/*controller.isRate.value*/, child: Padding(
                ...
                Obx(() => Row(
                  ...
                  children: [
                    InkWell(
                      ...
                      child: Image.asset(
                        controller.isChecked.value
                            ? ImagesConstant.ic_checked
                            : ImagesConstant.ic_unchecked,
                        width: 6.w,
                        height: 6.w,
                      ),
                    ),
                    ...
                    TitleTextWidget(StringConstant.in_original,
    ...
    return Card(
      ...
      shape: (controller.lastSelectedIndex.value == index && controller.lastItemIndex.value == itemIndex)
          ? RoundedRectangleBorder(
              side: new BorderSide(
                  color: ColorConstant.PrimaryColor, width: 0.5.w),
              borderRadius: BorderRadius.circular(3.w))
          : RoundedRectangleBorder(
              side: new BorderSide(color: ColorConstant.White, width: 0.5.w),
              borderRadius: BorderRadius.circular(3.w)),
      child: Padding(
        padding: EdgeInsets.all(1.w),
        ...
        child: Stack(
          children: [
            ...
            Visibility(
              visible: (widget.list[itemIndex].effects[index]
                  .endsWith("-transform")),
              child: Positioned(
                right: 1.5.w,
                top: 0.4.h,
            ...
            if (controller.isChecked.value &&
                tempData.contains(widget.list[itemIndex].effects[index]))
              Positioned(
                bottom: 0.4.h,
                left: 1.5.w,
  ...
  Future<void> getCartoon(BuildContext context) async {
    ...
    var key = (controller.isChecked.value && tempData.contains(widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value])) ? widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value] + "-original_face" : widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value];
        ...
        if (res.statusCode == 200) {
          ...
          if (tokenResponse.statusCode == 200) {
            if (tokenParsed['data'] == null) {
              ...
              params.add(JsonValueModel("algoname", (controller.isChecked.value && tempData.contains(widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value])) ? widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value] + "-original_face" : widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value]));
              ...
              var databody = jsonEncode(<String, dynamic>{
                ...
                'is_data': 0,
                'algoname': (controller.isChecked.value && tempData.contains(widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value])) ? widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value] + "-original_face" : widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value],
                'direct': 1,
                's': sToken(params),
              });
              ...
              if (cartoonizeResponse.statusCode == 200) {
                ...
                if (parsed['data'].toString().startsWith('<')) {
                  ...
                  get(Uri.parse('https://socialbook.io/api/log/cartoonize?algoname=${widget.list[controller.lastItemIndex.value].effects[controller.lastSelectedIndex.value]}'), headers: headers);
                }
              } else {
                controller.changeIsLoading(false);
  ...
  Future<void> likeDislike(bool like) async {
    ...
    await post(Uri.parse('https://socialbook.io/api/tool/matting/evaluate'), body: databody, headers: (sharedPrefs.getBool("isLogin") ?? false) ? headers : headers1).whenComplete(() async {
    controller.changeIsLoading(false);
  ...
  Widget _buildTextItem(BuildContext context, int index) {
    return Obx(() => GestureDetector(
      ...
      child: Padding(
        ...
        child: TitleTextWidget((widget.list[index].display_name != "null")? widget.list[index].display_name : widget.list[index].key, (index == controller.lastItemIndex1.value)? ColorConstant.TextBlack : ColorConstant.LightTextColor, (index == controller.lastItemIndex1.value)? FontWeight.w600 : FontWeight.w400, 12.sp),
      ),
    ));
  }
}
          ...
          child: Container(
            child: Column(
              ...
              children: [
                ...
                TitleTextWidget(StringConstant.signup_text1,
                    ColorConstant.TextBlack, FontWeight.w600, 14.sp),
                SizedBox(
                  height: 1.h,
                ),
                TitleTextWidget(StringConstant.signup_text2,
