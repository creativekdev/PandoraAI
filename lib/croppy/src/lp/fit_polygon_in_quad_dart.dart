...
void _addBasicConstraints(
  ...
  for (final d in dv) {
    ...
    for (final i in [0, 1, 2, 3]) {
      ...
      final constraint = cm(quadJ.x - quadI.x) * (yNew + _alphaY * cm(dy) - cm(quadI.y)) - cm(quadJ.y - quadI.y) * (xNew + alphaX * cm(dx) - cm(quadI.x)) <= cm(0);
...
Aabb2 fitPolygonInQuadOnResizeImpl(
  ...
  if (aspectRatio != null) {
    solver.addConstraint(
      (alphaX * cm(aabbSize.x) - alphaY * cm(aabbSize.y) * cm(aspectRatio)).equals(cm(0.0)),
    );
  }
